...
contract("NFTFarm", ([owner, investor]) => {
  ...
  describe("Farming NFTs", async () => {
    it("rewards stakers with points that they can use to redeem NFTs", async () => {
      ...
      await usdcToken.approve(nftFarm.address, tokens("1"), { from: investor });
      await nftFarm.stakeTokens(tokens("1"), { from: investor });
      const finalFarmTokenBal = await usdcToken.balanceOf(nftFarm.address);
      ...
      expect(+finalPointsBal).to.equal(
        +iniPointsBal.toString() + +tokens("700").toString()
      );
      ...
      expect(
        (+finalNFTCornBal.toString() + +finalNFTPotatoBal.toString()).toString()
      ).to.equal("5");

      // allows investor to unstake tokens
      const iniTokenBal = await usdcToken.balanceOf(investor);
      const stakedBal = (await nftFarm.userInfo(investor)).stakedAmount;
      await nftFarm.unstakeTokens({ from: investor });
      const finalTokenBal = await usdcToken.balanceOf(investor);
      expect(+finalTokenBal.toString()).to.equal(
        +iniTokenBal.toString() + +stakedBal.toString()
      );
    });
  });
});
